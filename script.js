...
// ===== Mobile controls: robust pointer bindings (Android + iOS) =====
(function mobileControls(){
  
  // نگاشت دکمه‌ها (طبق چیدمان جدید)
  const holdMap = [
    ["btnLeft",  "ArrowLeft"],
    ["btnRight", "ArrowRight"],

    // ▲ بالا سمت چپ = Jump (Space)
    ["btnJet",   "Space"],

    // JET سمت راست = Fly (ArrowUp)
    ["btnJump",  "ArrowUp"],
  ...
  holdMap.forEach(([id,code])=>bindHold(id,code));
  ...
  bindTap("btnStart", ()=>{ if(state===STATE.INTRO) startGame(); else resetAll(true); });
